...
mdl <- x ~ spde1D(map = x, 
                  model = inla.spde2.pcmatern(mesh1D, prior.range = c(150,0.75), prior.sigma = c(0.1,0.75))) + 
           Intercept
```

The function `bru( )` can not be used for point data. Here we want to fit to the actual points, and the `inlabru` function that
does this is called `lgcp` (for 'Log Gaussian Cox Process'). The The `ips' and `domain` parameters
are optional, one or the other or neither can be used. If neither are used, `lgcp` will infer the boundary of the `x` domain by 
using the upper and lower limits of the `x` column in `pts2`. Here are three ways of doing the same thing:


```{r results="hide",warning=FALSE,message=FALSE}
fit.spde <- lgcp(mdl, pts2, ips = ipoints(c(0,55), 50, name = "x"))
fita.spde <- lgcp(mdl, pts2, domain = list(x = mesh1D))
fitb.spde <- lgcp(mdl, pts2)
```
...
```{r results="hide",warning=FALSE,message=FALSE,echo=FALSE, eval=FALSE}
...
* `mce` is the estimated Monte Carlo error from sampling the posterior.
* `n.samples` is the number of samples from the posterior that were taken in order to calculate the above.
...
Compare the LGCP fit to a GAM fit
----------------------------------
...
```{r results="hide",warning=FALSE,message=FALSE,echo=FALSE,eval=TRUE}
plot(pred_spde) +  
  geom_point(data = pts2, aes(x = x),y = 0, pch = "|", cex = 2) + 
